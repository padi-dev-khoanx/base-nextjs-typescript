  ...
  return (
    <div className='flex justify-between border-b border-b-[#D9D9D9] py-[22px] pl-6 pr-[38px]'>
      ...
      >
        ...
        <span className='text-xl leading-[28px] font-medium truncate w-96'>{title}</span>
      </div>
      <div>
        {showButtonRight && (
          <Dropdown menu={{ items }} placement='bottomRight' trigger={['click']}>
            <a onClick={(e) => e.preventDefault()}>
              <button className='inline-flex items-center p-2 text-sm font-medium text-center text-gray-900 bg-white px-[17px] py-[6.4px] border border-[#D9D9D9] focus:outline-none focus:ring-gray-50'>
                <EllipsisOutlined />
              </button>
            </a>
          </Dropdown>
